...
class Eraser{
	...
	eraseLine(x_end, y_end){
    	...
    	var difference = Math.pow(x_dist, 2) + Math.pow(y_dist, 2);                             
    	difference = Math.sqrt(difference);
    	if(difference <= 10){															// Checks if line is done.
    		this.erase_count++;
    	} else {
...
function DEsetup(width, height){
	DEstage.addChild(graphics);
	DEstage.visible = false;
}

function DEresize(width, height, oldWidth, oldHeight){
	x_ratio = width/oldWidth;
	y_ratio = height/oldHeight;
	DEstage.scale.x = DEstage.scale.x * x_ratio;
	DEstage.scale.y = DEstage.scale.y * y_ratio; 
}
