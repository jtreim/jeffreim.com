...
function setup(){
	...
	if(window.innerWidth < 700){
		grass = new Grass(0, renderer.height*2/3, renderer.width, renderer.height/3, 800);
		butterfly = new Butterfly(renderer.width*7/8, renderer.height*7/8, -5);
		stick = new ControlStick(window.innerWidth/2, window.innerHeight*3/4, 20, 30, graphics);
	}else{
		grass = new Grass(0, renderer.height*2/3, renderer.width, renderer.height/3, 2000);
		butterfly = new Butterfly(renderer.width*7/8, renderer.height*7/8, -5);
...
class Butterfly{
	constructor(x, y, spd){
		...
		this.MAX_Y = renderer.height - this.MIN_Y;
		var scale = (y - this.MIN_Y)/(this.MAX_Y);
		scale *= BFLY_SCALE;
	...
	moveView(input){
		var scale = (this.sprite.y - this.MIN_Y)/(this.MAX_Y);

		this.sprite.x += input.x * scale;
		this.sprite.y += input.y * scale;

		scale = (this.sprite.y - this.MIN_Y)/(this.MAX_Y);
		scale *= BFLY_SCALE;
		this.scale.x = scale;
		this.scale.y = scale;
	}
